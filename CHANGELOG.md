...
- **A restored session — and every turn after a compaction — no longer starts over with no idea what just happened.** Whenever Cursor's own server-side conversation cannot be resumed, the whole session is rebuilt from Pi's transcript. That rebuild dropped the turn that was still in flight: `parseMessages` lifted the tool results out into `toolResults` and threw the turn itself away, and the request's user message was then computed as `userText || toolResults.join("\n")` — which fails in both directions. With a user question present the tool results were discarded and the model was asked the original question again, so it redid work it had already done; with no question present the model got raw tool output as if the user had typed it, so it had no task at all. After a split-turn compaction the second shape is exactly what Pi produces, and the compaction summary was handed over as though the user had just typed it. The in-flight turn — the assistant's calls and the results that came back — is now replayed as history, and the request says to continue from it rather than re-asking. When Cursor *does* resume from its own checkpoint our history is ignored, and that checkpoint has the calls but not their results (the stream died before delivery), so there the results travel in the request instead. This was a long-standing defect in the rebuild path, not a new one: it fired on session restore all along, and making compaction actually take effect is what turned it into something that happened on every turn. Message parsing moves to `cursor/message-parsing.ts` so it can be unit tested at all.
