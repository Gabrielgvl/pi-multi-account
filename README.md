...
- **Keeps canonical Codex children on the selected account.** With `childProxy` enabled, an extension-free child can request `openai-codex/<model>` with its original provider/model identity and payload; the parent-owned loopback chooses the currently healthy Codex account internally. If every Codex account is cooling down, the extension removes its own canonical loopback entry so the child can use Pi's native fallback instead of a dead 404 route. A user-owned canonical `openai-codex` entry in `models.json` is preserved rather than overwritten.
