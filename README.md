...
| `continueAfterCompaction` | `false` | After an **automatic** compaction, carry the task on instead of ending the run and waiting for the user to type "continue". This is Pi's own route, not a workaround: `_runAutoCompaction` ends with `return this.agent.hasQueuedMessages()` and the run loop turns that into `agent.continue()`, which drains the follow-up queue — Pi uses it for overflow recovery but queues nothing on the threshold path, so a threshold compaction always ends the run. Measured across 59 compactions in real logs: 49 % were followed by the user typing a short "continue", and most of the rest were the user pasting the summary back in by hand. One follow-up is queued, it shares the `maxAutoContinuesPerPrompt` budget with failover, and it is skipped for a manual `/compact`, when Pi is already retrying, when the session is idle, and after the user pressed Esc. The message tells the model that finishing is a valid answer, so a task that really was done is reported in one line rather than padded with invented work. `false` disables it. |
